...
void paging_dump_tables(struct dcb *dispatcher)
{
    ...
    if (!dispatcher->vspace) {
        printk(LOG_ERR, "dispatcher->vspace = 0x%"PRIxLPADDR": not set!\n",
               dispatcher->vspace);
        return;
    }
    if ((dispatcher->vspace & (ARM_L1_ALIGN-1)) != 0) {
        printk(LOG_ERR,
               "dispatcher->vspace = 0x%"PRIxLPADDR": not %d byte-aligned!\n",
               dispatcher->vspace, ARM_L1_ALIGN);
        return;
    }
    printf("dispatcher->vspace: %p\n", dispatcher->vspace);
    ...
    for (int l1_index = 0; l1_index < ARM_L1_MAX_ENTRIES; l1_index++) {
        ...
        if (l1_e->invalid.type == 2) { // section 
            ...
            printf("%d: (l2table) 0x%"PRIxGENPADDR" (raw=%#08"PRIx32")\n",
                    l1_index, ptable_gp, l1_e->raw);
