...
##########################################################################
# Copyright (c) 2009, 2011, 2013, 2015, 2016, ETH Zurich.
# All rights reserved.
#
...
SITECONFIG="default"
if [[ `hostname` == stud*-h5[67].inf.ethz.ch ]]; then
    SITECONFIG="cab"
fi
...
if test ! -f hake/Config.hs; then
    ...
    if [ ${SITECONFIG} != "default" ]; then
        echo "Applying siteconfig for site ${SITECONFIG}"
        patch -p1 hake/Config.hs $SRCDIR/hake/Config.hs.${SITECONFIG}.siteconfig.patch
	echo "Overriding TOOLROOT with '/pub/aos/tools'"
        TOOLROOT="Just \"/pub/aos/tools\""
    fi
